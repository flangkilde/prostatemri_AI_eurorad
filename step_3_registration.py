...
#
# Elastix and Transformix has to be installed before using the script.
...
for dir_entry in os.scandir(input_mask_path):
    ...
    if os.path.exists(os.path.join(output_path,'{:04d}_0001.nii.gz'.format(int(codenr)))):
        ...
        if os.path.exists('{}/{:04d}_0001.nii.gz'.format(input_path,int(codenr))):
            datacomplete=True
        else:
            datacomplete=False
